...
func main() {
    ...
    manager.GetCache("queries").Set("what is Go?", "a language", 10*time.Minute)
    manager.GetCache("embeddings").Set("text", []float32{0.1}, 1*time.Hour)
...
- [x] Phase 32: Project Hygiene & Documentation — `golangci-lint` (v2 config: errcheck/govet/ineffassign/staticcheck/unused + gosec, misspell, revive, unconvert) at **0 issues** repo-wide; CI workflows (`go.yml` with lint, tests, build, overall coverage gate, govulncheck, license check, and benchmark-regression detection; `tag.yml` for semver tag creation; `release.yml` for tag-triggered cross-platform releases via `scripts/release-build.sh`); `scripts/benchcompare.sh` benchmark baseline tool; governance docs (`CONTRIBUTING.md`, `SECURITY.md`, `CODE_OF_CONDUCT.md`, `ARCHITECTURE.md`, `GOVERNANCE.md`); per-package per-package READMEs (33 of 36 — `govern/`, `example/e2e/` and `example/production/` have none); runnable examples (`example/e2e` full-lifecycle tutorial, `example/production` server deployment via the typed client, `example/README.md`); `docs/BENCHMARKS.md` benchmark comparison guide and `docs/MIGRATION.md` upgrade guide; `CHANGELOG.md`
...
golangci-lint run           # static analysis (0 issues expected; see .golangci.yml)
go vet ./...                # standard vet
CGO_ENABLED=1 go test ./... -race   # race detector (needs a C compiler)
go run ./example/e2e        # end-to-end tutorial (deterministic, offline)
```
